...
const SubscriptionPlans = () => {
  ...
  useEffect(() => {
    const fetchSuperUserPaymentStatus = async () => {
      try {
        const response = await axios.get(`${API_BASE_URL}/fetch-subscriptions-details/${employeeId}/SuperUser`);
...
const isConfirmed = window.confirm('Are you sure you want to update the payment status to "Payment Completed"?');
...
setTimeout(() => {
  ...
  <p className="loading-text">Wait For Few Seconds<span className="dots">...</span></p>
</div>
            ...
            {/* Premium Card */}
            {
              ...
              plans.slice(0, 3).map((plan, index) => {
                ...
                return (
                  ...
                  >
                    <h4 className="card-title"><b>{plan.name}</b></h4>
                    <h2>{plan.price}</h2>
                    {
