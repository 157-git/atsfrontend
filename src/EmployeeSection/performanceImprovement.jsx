...
const PerformanceImprovement = ({ loginEmployeeName, onCloseIncentive, onBack }) => {
  ...
  const showModal1 = () => {
    // If userType is Recruiters, skip the first modal and show the job selection modal directly
    ...
    setIsModalOpen1(true); // Open second modal
  };
  ...
  // Rajlaxmi jagadale  updated the handleSelectAllNew function to properly handle selection of all items Date 09/04/2025
  const handleSelectAllNew = (role) => {
    let newIds = [];
    ...
    if (role === "Manager") {
      ...
      setSelectedTeamLeaders(
        teamLeadersList.map((leader) => ({
          ...
          teamLeaderJobRole: leader.jobRole,
        }))
      );
    } else if (role === "Recruiters") {
      newIds = recruitersList.map((recruiter) => recruiter.employeeId);
  ...
  // Rajlaxmi jagadale Updated the handleCheckboxChange function to properly handle individual checkbox selection Date 09/04/2025
  const handleCheckboxChange = (role, id, completeValueObject) => {
    // If changing roles, clear previous selections
    if (selectedRole !== role) {
      ...
      if (role === "Manager") {
        setSelectedManagers([
          {
            managerId: completeValueObject.managerId,
            managerJobRole: completeValueObject.jobRole,
          },
        ])
        setSelectedTeamLeaders([])
        setSelectedRecruiters([])
        ...
        setSelectedTeamLeaders([
          {
            teamLeaderId: completeValueObject.teamLeaderId,
            teamLeaderJobRole: completeValueObject.jobRole,
          },
        ])
        setSelectedManagers([])
        setSelectedRecruiters([])
        ...
        setSelectedRecruiters([
          {
            recruiterId: completeValueObject.employeeId,
            recruiterJobRole: completeValueObject.jobRole,
          },
        ])
        setSelectedManagers([])
        setSelectedTeamLeaders([])
    ...
    // Handle toggling selection within the same role
    const isSelected = selectedIds.includes(id)

    // Update selectedIds
    setSelectedIds((prevIds) => (isSelected ? prevIds.filter((selectedId) => selectedId !== id) : [...prevIds, id]))

    // Update the appropriate selection state based on role
    if (role === "Manager") {
      setSelectedManagers((prev) =>
        isSelected
          ? prev.filter((item) => item.managerId !== completeValueObject.managerId)
          : [
            ...prev,
            {
              managerId: completeValueObject.managerId,
      ...
      setSelectedTeamLeaders((prev) =>
        isSelected
          ? prev.filter((item) => item.teamLeaderId !== completeValueObject.teamLeaderId)
          : [
            ...prev,
            {
              teamLeaderId: completeValueObject.teamLeaderId,
      ...
      setSelectedRecruiters((prev) =>
        isSelected
          ? prev.filter((item) => item.recruiterId !== completeValueObject.employeeId)
          : [
            ...prev,
            {
              recruiterId: completeValueObject.employeeId,
  ...
  // Rajlaxmi Jagadale Updated that code date 11/04/2025
  const downloadChart = () => {
    ...
    if (chartInstance) {
      ...
      setTimeout(() => {
        ...
        link.href = imageUrl;
        link.download = `${loginEmployeeName}_PerformanceImprovement.png`;
  ...
  const isAllJobIdsSelected =
    selectedJobId.length === sortedUniqueClientDetails.length &&
    sortedUniqueClientDetails.length > 0;

  const handleSelectAllJobIdsForPerformance = () => {
    if (isAllJobIdsSelected) {
      // Deselect all
      setSelectedJobId([]);
    } else {
      // Select all
      setSelectedJobId([...sortedUniqueClientDetails]);
    }
  };
      ...
      <div className="PIE-Header-Sectionteamperformance">
        <div className="PIE-grid-dropdownteamperformance">
          <div className="PIE-dropdown-containerteamperformance">
            ...
            {displayModalContainer && userType !== "Recruiters" && (
              <>
                <Modal
                  width={1000}
        ...
        <div className="PIE-grid-dateteamperformance">
          ...
          <div className="PIE-job-filterteamperformance">
            ...
            <Modal
              title={
                <div className="PIE-jobId-modal-headteamperformance">
                  <span>Select Job Ids</span>

                  <button
                    className="PIE-selectAll-modal-btnteamperformance"
                    onClick={handleSelectAllJobIdsForPerformance}
                    disabled={sortedUniqueClientDetails.length === 0}
                  >
                    {selectedJobId.length === sortedUniqueClientDetails.length
                      ? "Clear All"
                      : "Select All"}
        ...
        <>
          ...
          <div className="procees-time-tableteamperformance">
            ...
            <div className="chart-display">
              <Bar className="chartdispaly" ref={chartRef} data={chartData} options={options} />
            </div>
