...
const EmployeeMasterSheet = () => {
  ...
  const [employeeMasterSheet, setEmployeeMasterSheet] = useState([]);
  ...
  const [showFilterSection, setShowFilterSection] = useState(false); // Ajhar_tamboli/EmployeeMasterSheet/FiterButton/10-09-24
  const [selectedEmployeeId, setSelectedEmployeeId] = useState(null);
  const [filterOptions, setFilterOptions] = useState([]);
  const [filteredEmployeeMasterSheet, setFilteredEmployeeMasterSheet] =
    useState([]);
  ...
  const fetchData = async () => {
    try {
      ...
      console.log(data)
      setData(data);
      setFilteredEmployeeMasterSheet(data);
      setLoading(false);
    } catch (error) {
      console.error("Error fetching shortlisted data:", error);
    }
  ...
  // Ajhar_tamboli/EmployeeMasterSheet/FiterButton/10-09-24
  ...
  const filterData = () => {
    let filteredData = [...filteredEmployeeMasterSheet];
    ...
    setFilteredEmployeeMasterSheet(filteredData);
  };
  useEffect(() => {
    const options = Object.keys(filteredEmployeeMasterSheet[0] || {}).filter(
      (key) => limitedOptions.includes(key)
    );    
    setFilterOptions(options);
  }, [filteredEmployeeMasterSheet]);
...
<div className="emp-Share-N-filter">
            {userType !== "Recruiters" && (
              <div>
                {showShareButton ? (
                  <button
            ...
            <div className="filter-section">
              {filterOptions.map((option) => { 
                const uniqueValues = Array.from(
                  new Set(employeeMasterSheet.map((item) => item[option]))
                );
                return (
