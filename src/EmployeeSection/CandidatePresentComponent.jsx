...
const CandidatePresentComponent = ({ candidateData, onClose }) => {
  ...
  const getStatusMessage = () => {
    ...
    const { selectYesOrNo, finalStatus, recruiterName, candidateAddedTime } =
      candidateData;
    ...
    switch (finalStatus) {
      case "Hold":
        return (
          <span>
            This candidate's application is on Hold. Added by {recruiterName} on{" "}
            {candidateAddedTime}.
          </span>
        );
      case "Rejected":
        return (
          <span>
            This candidate's application was Rejected. Added by {recruiterName}{" "}
            on {candidateAddedTime}.
          </span>
        );
      case "No Response":
        return (
          <span>
            This candidate did not respond to further communication. Added by{" "}
            {recruiterName} on {candidateAddedTime}.
          </span>
        );
      case "Back Out":
        return (
          <span>
            This candidate backed out of the process. Added by {recruiterName}{" "}
            on {candidateAddedTime}.
          </span>
        );
      case "Pending":
        return (
          <span>
            This candidate's application is Pending. Added by {recruiterName} on{" "}
            {candidateAddedTime}.
          </span>
        );
      default:
        return (
          <strong>
            Application status:{" "}
            <span className="status-green1">{finalStatus}</span>. Added by{" "}
            {recruiterName} on {candidateAddedTime}.
          </strong>
        );
    }
          ...
          {candidateData ? (
            <div className="candidate-info">
              ...
              <h4 className="recruiter-name">
                Recruiter Name: {candidateData.recruiterName}
              </h4>
              <p className="candidate-description">{getStatusMessage()}</p>
              <div className="job-info-candidate">
                <div className="compdesdiv1">
                  <p className="paragraphWidth50">
                    <b>Job ID:</b> {candidateData.requirementId}
                  </p>
                  <p className="paragraphWidth50">
                    <b>Company:</b> {candidateData.requirementCompany}
                  </p>
                </div>
              </div>
              <div className="compdesdiv1">
                <p className="paragraphWidth50">
                  <b>Designation:</b> {candidateData.jobDesignation}
                </p>
                <p
                  className={`candidate-status ${getStatusClass(
                    candidateData.finalStatus
                  )} paragraphWidth50`}
                >
