...
function JobDescriptionEdm({
  ...
  useEffect(() => {
    // If Descriptions is missing, use template data and DO NOT call API
    if (!Descriptions) {
      if (!descriptionFromTempGen) return;

      setData({
        ...descriptionFromTempGen,
        skills: Array.isArray(descriptionFromTempGen.skills)
          ? descriptionFromTempGen.skills.join(", ")
          : descriptionFromTempGen.skills,
      });
      return;
    }

    // Only call API when Descriptions exists
    fetch(
  ...
  const togglePlay = () => {
    ...
    if (!isPlaying) {
      ...
      if (voices.length > 0) {
        ...
        // Apply voice in priority order
        utterance.voice =
          indianVoice || ukFemaleVoice || usFemaleVoice || voices[0];
  ...
  const generateAndDownloadVideo = async () => {
    setDownloadImg(true);
    try {
      const element = document.getElementById("shareEMD");
      ...
      a.href = url;
      a.download = "job_description.mp4";
      a.click();
          ...
          <div contentEditable="true" className="job-posting" id="shareEMD">
            ...
            <div className="details">
              ...
              <div className="skill-content">
                ...
                <p className="share-edm-skill">Week Offs : - {data.weekOff}</p>
                {/* <p className="share-edm-skill">
                    Pick-up and Drop facility available.
                  </p> */}
              </div>{" "}
