...
const Attendance = ({ reportDataDatewise, selectedIdsProp, selectedJobRole, finalStartDatePropState, finalEndDatePropState, loginEmployeeName }) => {
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      const newbarchartclassforsetstyles = document.getElementsByClassName('newbarchartclassforsetstyles');
  ...
  const [displaycandidatedatareporttable, setdisplaycandatareport] = useState(false);
  ...
  setTimeout(() => {
    if (sahilDivRef.current) {
      sahilDivRef.current.scrollIntoView({ behavior: "smooth", block: "start" });
    }
