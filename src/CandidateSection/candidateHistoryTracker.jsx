...
/* Rajlaxmi JAgadale Upadted taht code Date 08/04/2025*/
  ...
  switch (selectedRange) {
    case "Current Month":
      ...
      setCustomStartDate(""); // optional
      setCustomEndDate("");   // optional
      start = startOfMonth(today);
      end = today;
      break;
 ...
 const handleCustomStartDateChange = (e) => {
  ...
  if (customEndDate && new Date(value) > new Date(customEndDate)) {
  ...
  if (customStartDate && new Date(customStartDate) > new Date(value)) {
    setDateError("Start date cannot be later than end date !");
  } else {
    setDateError(""); // Clear error
  }
...
// Rajlaxmi JAgadale Chanegs here date 14/04/2025
  ...
  <div className="tracker-date-report-option">
    <div className="histry-date-div">
      ...
      {["Current Month", "Last Month", "Last 3 Months", "Last 6 Months", "Last 1 Year", "custom"].map((option) => (
        <label
          key={option}
          className={`PI-radio-label ${dateRange === option ? "selected" : ""}`}
        >
          <input
            type="radio"
            value={option}
            ...
            onChange={handleDateRangeChange}
          />
          {option === "custom" ? "Custom Date" : option}
        </label>
      ))}
    </div>

    {/* ⬇️ Show this only if a duration is selected */}
    {dateRange && (
      <>
        <center>
          <div className="history-tracker-custom-dates">
  ...
  <div style={{ color: "red", marginTop: "10px", fontWeight: "bold" }}>
    {dateError}
        ...
        <div style={{ marginTop: 0 }} className="history-button-main-div">
          <div className="history-button-container">
            {[
              "Yet To Confirm", "Interview Schedule", "Attending After Some time",
              "Shortlisted For Hr Round", "Shortlisted For Technical Round", "Shortlisted For L1 Round",
              "Inactive", "Shortlisted For L2 Round", "Shortlisted For L3 Round",
              "Selected", "Rejected", "Hold", "Joining", "Drop Out",
              "To Join", "Joined", "Not Joined", "No Show", "Active"
            ].map((label) => (
              <div key={label} className="bha-dropdown">
                <button
                  className={`bhafilter-button ${openDropdown === label ? "active" : ""}`}
              ...
              <div className="can-history-filter-data-content">
                <div className="can-history-data-table-container">
                  <table className="can-history-data-table">
                    ...
                    <tbody>
  {(() => {
    const filteredData = calculatePercentages(sortedData).filter(
      (item) =>
      ...
      <tr key={index}>
        ...
        <td style={{ textAlign: "center" }}>
          {(() => {
            ...
            const firstNonNullValue = fields.find((v) => v != null) ?? 0;
            const percentage = ((firstNonNullValue / (item.totalCount ?? 1)) * 100).toFixed(2);
            return item.totalCount ? `${percentage} %` : "N/A";
          })()}
        </td>
